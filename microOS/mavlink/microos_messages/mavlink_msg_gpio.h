...
typedef struct __mavlink_gpio_t
{
 ...
 uint8_t ID; ///< ID of the debugging message: multiple debugging messages at once.
} mavlink_gpio_t;

#define MAVLINK_MSG_ID_GPIO_LEN 53
#define MAVLINK_MSG_ID_4_LEN 53

#define MAVLINK_MSG_ID_GPIO_CRC 92
#define MAVLINK_MSG_ID_4_CRC 92
...
#define MAVLINK_MESSAGE_INFO_GPIO { \
	...
	{  { "time", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_gpio_t, time) }, \
         ...
         { "ID", NULL, MAVLINK_TYPE_UINT8_T, 0, 52, offsetof(mavlink_gpio_t, ID) }, \
         } \
}
 ...
 * @param ID ID of the debugging message: multiple debugging messages at once.
...
static inline uint16_t mavlink_msg_gpio_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint32_t time, uint8_t ID, const float *gpio_float, const int32_t *gpio_int)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_GPIO_LEN];
	_mav_put_uint32_t(buf, 0, time);
	_mav_put_uint8_t(buf, 52, ID);
...
{
	return mavlink_msg_gpio_pack(system_id, component_id, msg, gpio->time, gpio->ID, gpio->gpio_float, gpio->gpio_int);
}
...
{
	return mavlink_msg_gpio_pack_chan(system_id, component_id, chan, msg, gpio->time, gpio->ID, gpio->gpio_float, gpio->gpio_int);
}
...
static inline void mavlink_msg_gpio_send(mavlink_channel_t chan, uint32_t time, uint8_t ID, const float *gpio_float, const int32_t *gpio_int)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_GPIO_LEN];
	_mav_put_uint32_t(buf, 0, time);
 ...
 */
static inline void mavlink_msg_gpio_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t time, uint8_t ID, const float *gpio_float, const int32_t *gpio_int)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field ID from gpio message
 *
 * @return ID of the debugging message: multiple debugging messages at once.
 */
static inline uint8_t mavlink_msg_gpio_get_ID(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_gpio_decode(const mavlink_message_t* msg, mavlink_gpio_t* gpio)
{
	...
	gpio->ID = mavlink_msg_gpio_get_ID(msg);
#else
